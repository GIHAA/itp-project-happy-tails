...
const deleteeamount = (async (req, res) => {
  try {
      ...
      if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
          return res.status(400).json({ error: 'Invalid income expenses of event ID' });
      }

      // Check if the income expenses of event exists
      const deleteeamount = await inExEvent.findById(req.params.id);
      if (!deleteeamount) {
          return res.status(404).json({ error: 'income expenses of event not found' });
      }
      // Delete the income expenses of event
      await inExEvent.findByIdAndRemove(req.params.id);

      return res.status(200).json({ message: 'income expenses of event deleted successfully', deleteeamount });
  } catch (err) {
...
// }
  try {
      ...
      if (!eamount) {
          return res.status(404).send({ error: 'income expenses of event not found' });
      }

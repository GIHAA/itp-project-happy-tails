...
const addcusDonation = asyncHandler(async (req, res) => {
  const { cus_id, description, price, payment_date, payment_time, status } =
    req.body;
  ...
  Customer
    ...
    : res.status(400).json({ message: "cusDonation not created" });
});
...
const updatecusDonation = asyncHandler(async (req, res) => {
  ...
  Customer
    ? res.status(201).json(Customer)
    : res.status(400).json({ message: "cusDonation not updated" });
});
...
const deletecusDonation = asyncHandler(async (req, res) => {
  ...
  Customer
    ...
    : res.status(400).json({ message: "cusDonation not deleted" });
});

...
const addSuppliers = asyncHandler(async (req, res) => {
  ...
  supplier
    ...
    : res.status(400).json({ message: "Supplier did not inserted" });
});
...
const updateSuppliers = async (req, res) => {
  ...
  supplier
    ? res.status(201).json(supplier)
    : res.status(400).json({ message: "Supplier details not updated" });
};
...
const deleteSuppliers = asyncHandler(async (req, res) => {
  ...
  supplier
    ? res.status(200).json(supplier)
    : res.status(400).json({ message: "Supplier details not deleted" });
});

...
const [eventAmount,setEventAmount] = useState({});
 const [amountName,setAmountName] = useState("")
    ...
    const [button, setButton] = useState(true);
     ...
     useEffect(()=>{
        ...
        async function getevent(){
          ...
          const amount = allEventAmounts.filter((ea) => ea.eid === res.data.even.eid)[0];
         // console.log(amount)
  ...
  console.log(eid)
    ...
    Promise.all([
      axios.post("http://localhost:8080/api/eventregister/addbooking", newregister),
   ...
   /* const handleQRCodeModalClose = () => {
    ...
    const QRCodeContent = ({ cusName, eventName, noOfTicket, dbprice, total }) => {
      return (
        <html>
          <head>
...
async function checkavailable(event, selectedValue) {
  const response = await axios.get("http://localhost:8080/api/eventregister/getbooking");
  ...
  if (selectedValue > remaining) {
    if (remaining <= 10) {
      toast.warning(`Only ${remaining} tickets are available. Please select a smaller number.`);
      setButton(false)
    } else {
  ...
  return (
    ...
    <div className="w-1/2 bg-white rounded-lg shadow-2xl p-8 m-4" style={{ backgroundColor: "#2E4960" }}>
        <h1 class="block w-full text-center text-white text-2xl font-bold mb-6">Register Event</h1>
        ...
        <div class="flex flex-col mb-4 block">
            ...
            <div class="flex flex-col mb-4 block">
                ...
                <input class="border py-2 px-3 text-grey-800 rounded-lg" style={{ width: "65%", alignSelf:'center' }} type="text" name="cusName" id="cusName" required onChange={(e)=>{
  ...
  <div className="w-1/2 pl-2">
    ...
    <input className="border py-2 px-3 text-grey-800 rounded-lg w-full" type="text" name="total" id="total" value={dbprice * noOfTicket} disabled onChange={(e) => setTotal(e.target.value)} />
            ...
            <div class="flex flex-col mb-4">
                ...
                <input class="border py-2 px-3 text-grey-800 rounded-lg" style={{ width: "65%", alignSelf:'center' }} type="email" name="email" id="email" onChange={(e)=>{
  ...
  >
    Register Event
        ...
        <div class="bg-white p-4 rounded-lg">
          ...
          <div class="text-red text-center">Please take photo or download QR code</div>

...
export default function EditBooking() {
    ...
    async function getbooking() {
      try {
        const res = await axios.get(
          `http://localhost:8080/api/eventregister/getbooking/${id}`
        );
  ...
  useEffect(() => {
    async function getEvent() {
      try {
        ...
        if (eventName) {
          const targetEvent = res.data.allevents.find(
            (e) => e.name === eventName
          );
          if (targetEvent) {
  ...
  async function EditBooking(e) {
    ...
    try {
      ...
      Promise.all([
        axios.put(
          `http://localhost:8080/api/eventregister/editBooking/${id}`,
          newBooking
        ),
        axios.put(

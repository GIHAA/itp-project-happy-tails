...
useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post("http://localhost:8080/api/eventstock/reqStock",request)
      .then(res => {
  ...
  return (
    ...
    <Link to="/eventdashboard/stock" style={{color:'white'}}>Back</Link>
    </div>
    <h1 class="block w-full text-center text-white text-2xl font-bold mb-6">Request Stock</h1>
...
<div class="flex flex-col mb-4 block">
            <label class="mb-2 text-lg text-white" style={{marginLeft:'17.5%'}} htmlFor={`quantity${index}`}>
              Quantity {index + 1}
            </label>
            <input class="border py-2 px-3 text-grey-800 rounded-lg" style={{ width: "65%", alignSelf:'center' }} type="number" min={0} name="quantity" id={`quantity${index}`} value={item.quantity} onChange={(e) => handleItemChange(index, e)} placeholder="Enter quantity needed" required  onKeyPress={(e) => {                                           /* to restrict other character and accept only integer*/
...
<div style={{marginTop:'20px'}}>
        <button className="block bg-green-500 hover:bg-green-700 text-white font-bold uppercase text-lg mx-auto p-2 rounded-lg" type="submit">
          Submit Request
        </button>
        </div>
      </form>

...
export default function EditEvent(){
    ...
    useEffect(()=>{
      async function getEventData() {
        try {
          const eventRes = await axios.get(`http://localhost:8080/api/event/getEvent/${id}`);
          ...
          const amount = allEventAmounts.filter((ea) => ea.eid === oneEvent.even.eid)[0];
          console.log(amount)
...
*/
 ...
 //  console.log(time)
  ...
  function convertToBase64(e) {
    ...
    <h1 class="block w-full text-center text-white text-2xl font-bold mb-6">Edit Event</h1>
        <form onSubmit={EditEvent}>
            <div class="flex flex-col mb-4 block">
                ...
                <input class="border py-2 px-3 text-grey-800 rounded-lg" style={{ width: "65%", alignSelf:'center' }} type="text" name="name" id="name" value={name} required onChange={(e)=>{
        ...
        setDesc(e.target.value)}}/>
            ...
            <div class="flex flex-col mb-4">
            <label className="block text-lg text-white mb-2" style={{marginLeft:'17.5%'}} htmlFor="status">Status</label>
    <select className="border py-2 px-3 text-grey-800 rounded-lg w-full" name="status" id="status" style={{ width: "65%", alignSelf:'center' }}  required onChange={(e) => setStatus(e.target.value)
    }>
      ...
      <option value="Finished">Finished</option>
            ...
            <div class="flex flex-col mb-4" >
                ...
                <input class="border py-2 px-3 text-grey-800 rounded-lg" style={{ width: "65%", alignSelf:'center' }} type="date" name="date" id="date" defaultValue={date} required onChange={(e)=>{
        setDate(e.target.value)}}/>
            </div>
            <div class="flex flex-col mb-4">
                <label class="mb-2 text-lg text-white" style={{marginLeft:'17.5%'}} for="venue">Venue</label>
                <input class="border py-2 px-3 text-grey-800 rounded-lg" style={{ width: "65%", alignSelf:'center' }} type="text" name="venue" id="venue" value={venue} required onChange={(e)=>{
        setVenue(e.target.value)}}/>
            </div>
            <div class="flex flex-col mb-4">
                <label class="mb-2 text-lg text-white" style={{marginLeft:'17.5%'}} for="price">Price</label>
                <input class="border py-2 px-3 text-grey-800 rounded-lg" style={{ width: "65%", alignSelf:'center' }} type="number" min="0" step="1" name="price" id="price" value={price} required onChange={(e)=>{
        setPrice(e.target.value)}}/>

            <div class="flex flex-col mb-4">
                <label class="mb-2 text-lg text-white" style={{marginLeft:'17.5%'}} for="size">Participant Size</label>
                <input class="border py-2 px-3 text-grey-800 rounded-lg" style={{ width: "65%", alignSelf:'center' }} type="text" name="size" id="size" value={size} required onChange={(e)=>{
  ...
  {image == "" || image == null ? "" : <img width={100} height={100} src={image} style={{marginLeft:'17.5%',marginTop:'10px'}} />}
            ...
            <button style={{ backgroundColor: '#F2994A' }} className="block bg-teal-400 hover:bg-teal-600 text-white font-bold uppercase text-lg mx-auto p-2 rounded-lg" type="submit">Update Event</button>

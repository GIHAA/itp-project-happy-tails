...
const EventDashboard = () => {
  ...
  useEffect(() => {
      ...
      try {
        ...
        feedbacks.forEach((feedback) => {
          ...
          if (eventName in ratingByEvent) {
            ratingByEvent[eventName].totalRating = Number(ratingByEvent[eventName].totalRating) + Number(rating);
    ...
    const getStock = async () => {
      try {
        const res = await axios.get("http://localhost:8080/api/eventstock/getStocks");
     //   console.log(res.data.alleamount);
  ...
  useEffect(() => {
   ...
   //     console.log(finishCount)
    ...
    const [pendingCount, availableCount, finishCount] = eventStatusData.reduce(
      ([pending, available, finished], [pendingCount, availableCount, finishCount]) => [    pending + pendingCount,    available + availableCount,    finished + finishCount,  ],
      [0, 0, 0]
    );
...
const eventStatus = [
  ...
  useEffect(() => {
    const allbudgets = async () => {
      try {
        const res = await axios.get("http://localhost:8080/api/eventbudget/getBudgets");
        setBudget(res.data.allbudget);
        console.log(res.data.allbudget);

        const allBudget = res.data.allbudget.filter((bud) => bud.status === "Accepted" && bud.amountStatus === "Paid");
       setFilterBudget(allBudget);

       const total = allBudget.map((bud) => parseInt(bud.total));
       const totalbud = total.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
  ...
  useEffect(() => {
    ...
    const [pendingCount, acceptCount, rejectCount] = budgetStatusData.reduce(
      ([pending, accepted, rejected], [pendingCount, acceptCount, rejectCount]) => [    pending + pendingCount,    accepted + acceptCount,    rejected + rejectCount,  ],
      [0, 0, 0]
    );
...
},
{
  ...
  useEffect(() => {
    const chartData = datas.map(({ eventName, rate }) => ({ name: eventName, Rate: Number(rate) }));
...
<div style={{ display: 'flex' }}>
  <div style={{ flex: 1 }}>
    <div style={{ textAlign: 'center' }}>
      <h1 style={{textAlign:'left', marginLeft:'200px', fontWeight: 'bold' }}>Income - Expense Pie Chart</h1>
      {eventAmount ? (
  ...
  <div style={{ display: 'flex' }}>
    <div style={{ flex: 1 }}>
      <div style={{ textAlign: 'center' }}>
        <h1 style={{textAlign:'left', marginLeft:'200px', fontWeight: 'bold'}}>Event Status Bar Chart</h1>
        <div style={{ width: '100%' }}>
          <BarChart
            width={500}
            height={300}
...
<div style={{ display: 'flex' }}>
  <div style={{ flex: 1 }}>
    <div style={{ textAlign: 'center' }}>
      <h1 style={{textAlign:'left', marginLeft:'200px', fontWeight: 'bold'}}>Budget Request Status Bar Chart</h1>
      <div style={{ width: '100%' }}>
        <BarChart
          width={500}
          height={300}
...
<div style={{ display: 'flex' }}>
  <div style={{ flex: 1 }}>
    <div style={{ textAlign: 'center' }}>
      <h1 style={{textAlign:'left', marginLeft:'200px', fontWeight: 'bold'}}>Stock Request Status Bar Chart</h1>
      <div style={{ width: '100%' }}>
        <BarChart
          width={500}
          height={300}

...
const Report = () => {
    ...
    useEffect(() => {
      async function getevents() {
        try {
          const res3 = await axios.get("http://localhost:8080/api/event/getEvents");
...
doc.addPage();
  ...
  doc.text('All Event Report', doc.internal.pageSize.width / 2, 50, { align: 'center' });
  ...
  <span style={{ marginRight: '10px', fontSize:'20px',  }}>All Events</span>
   <button style={{ backgroundColor: '#459DE8' }} className="block bg-teal-400 hover:bg-teal-600 text-white font-bold uppercase text mx-auto p-2 rounded-full w-40" onClick={generatePDF}>Download PDF</button>
    ...
    events.map((event,index)=>(
            ...
            <td colSpan="7" style={{backgroundColor:'#D5ABAF', fontSize:'20px'}}>No events found.</td>

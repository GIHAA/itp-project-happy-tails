...
const AllEventBudget = () => {
  ...
  const onDelete =(id)=>{
    ...
    // }
    axios.delete(`http://localhost:8080/api/eventbudget/deleteBudget/${id}`).then((res)=>{
...
const handleTextSearch = (e) => {
  ...
  axios.get("http://localhost:8080/api/eventbudget/getBudgets").then((res) => {
...
<>
    ...
    <div class="flex flex-row-reverse space-x-1 space-x-reverse ...">
      <div className="mb-4">
  <button style={{ backgroundColor: '#1FE23F' }} className="block bg-teal-400 hover:bg-teal-600 text-white font-bold uppercase text mx-auto p-2 rounded-full w-40">
    <Link to={"/eventdashboard/addBudget"}>Request Budget</Link>
      ...
      {budgetRequests.length > 0 ? (
         ...
         <tr>
           ...
           <th scope="col" class="px-6 py-3" style={{color: 'white'}}>Total</th>
           <th scope="col" class="px-6 py-3" style={{color: 'white'}}>Status</th>
           <th scope="col" class="px-6 py-3" style={{color: 'white'}}>Amount Status</th>
       ...
       <tbody>
         {budgetRequests.map((request, index) => (
           <tr key={request.eventName} class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600" style={{backgroundColor: '#DBD9D5'}}>
             ...
             <td class="px-6 py-4" style={{color: 'black'}}>{request.budgetid}</td>
             ...
             {request.items.map((item, itemIndex) => (
               <tr key={itemIndex} >
                 <td  style={{color: 'black'}}>{item.product} - {item.amount}</td>
                {/** <td class="px-6 py-4" style={{color: 'black'}}>{item.amount}</td>*/} 
               </tr>
             ))}
            <td class="px-6 py-4" style={{color: 'black'}}>{request.total}</td>
            <td class="px-6 py-4" style={{color: 'black'}}>{request.status}</td>
            <td class="px-6 py-4" style={{color: 'black'}}>{request.amountStatus}</td>
            <td class="px-6 py-4" style={{color: 'black'}}>
            <div  className="flex justify-center">
      
            {request.status === "Accepted" && request.amountStatus === "Paid" ? (
  <div className="flex justify-center">
    <button style={{ backgroundColor: '#99ccff',marginRight:'20px' }} className="block bg-teal-400 hover:bg-teal-600 text-white font-bold uppercase text mx-auto p-2 rounded-full w-20" onClick={() =>  toast.error("This event has already been accepted and paid by the financial manager. You cannot edit this.")}>Edit</button>
    <button style={{ backgroundColor: '#ff9980' }} className=" bg-teal-400 hover:bg-teal-600 text-white font-bold uppercase text mx-auto p-2 rounded-full w-20" onClick={() => toast.error("This event has already been accepted and paid by the financial manager. You cannot delete this.")}>Delete</button>
  </div>
) : (
  <div className="flex justify-center">
    
      <button style={{ backgroundColor: '#459DE8',marginRight:'20px' }} className="block bg-teal-400 hover:bg-teal-600 text-white font-bold uppercase text mx-auto p-2 rounded-full w-20">
        <Link to={`/eventdashboard/editbudget/${request._id}`}>Edit</Link>
      </button>
    <button style={{ backgroundColor: '#D12222' }} className=" bg-teal-400 hover:bg-teal-600 text-white font-bold uppercase text mx-auto p-2 rounded-full w-20" onClick={()=>handleDelete(request._id)}>Delete</button>
  </div>
)}
        ...
        <p style={{backgroundColor:'#D5ABAF'}}>No budget requests found.</p>
      )}
    </div>
    </div>
    </>  

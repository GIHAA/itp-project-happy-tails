...
export default function UpdateVac() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await axios.put(
        `http://localhost:8080/api/health/reportupdate/${pid}`,
        newreport
      );
          ...
          {loading ? (
            ...
            <>
              <div class="flex flex-row items-center space-x-16">
                ...
                <div className="flex flex-col">
                  <label htmlFor="dateGiven" className="mb-2 font-bold">
                    Date Given:
                  </label>
                  <input
                    id="dateGiven"
                ...
                <div className="flex flex-col">
                  <label htmlFor="expirationDate" className="mb-2 font-bold">
                    Expiration Date:
                  </label>
                  <input
                    id="expirationDate"
                    ...
                    onChange={(e) => {
                      const updatedVaccinations = [...vaccinations];
                      updatedVaccinations[index].expirationDate =
                        e.target.value;
                      setVaccinations(updatedVaccinations);
                    }}
                    className="p-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"

...
export default function Breed() {
  ...
  async function UpdateData(e) {
    ...
    try {
      ...
      await axios.put(
        `http://localhost:8080/api/petbreed/breedupdate/${upId}`,
        newbreed
      );
      ...
      <div>
        ...
        <div style={{ display: "flex", justifyContent: "center" }}>
          <button
            ...
            onClick={() => {
              axios
                .delete(`http://localhost:8080/api/petbreed/deletebreed/${id}`)
                .then((res) => {})
                .catch((err) => {
                  toast.warning(err);
                });
  ...
  return (
    <div>
      <button
        onClick={openModal}
        className="bg-blue-500 mt-[150px] ml-[1150px] hover:bg-blue-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
      >
        ADD Breed
      </button>
      ...
      >
        <form
          className="bg-white shadow-md rounded px-8 ml-[150px] pt-6 pb-8 mb-4"
          onSubmit={addbreed}
        >
          <div className="mb-4">
            <label
      ...
      <Modal
        isOpen={upModelIsOpen}
        className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 transition-opacity"
      >
        <form
          className="bg-white shadow-md  ml-[150px] rounded px-8 pt-6 pb-8 mb-4"
          onSubmit={UpdateData}
        >
          <div className="mb-4">
            <label

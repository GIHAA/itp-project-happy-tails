...
const GetBooking = () => {
  ...
  function filterContent(book, searchTerm) {
    ...
    const result = book.filter(
      (r) =>
        r.cus_name.toLowerCase().includes(searchTerm.toLowerCase()) ||
        r.bid.toLowerCase().includes(searchTerm.toLowerCase())
    );
    setpayData(result);
  }
        ...
        <table class="mx-auto mt-10 max-w-5xl shadow-2xl w-full whitespace-nowrap rounded-lg bg-gray-300 divide-y divide-gray-300 overflow-hidden table-auto">
          ...
          <tbody class="divide-y divide-gray-400">
            {payData
              .filter(
                (data) =>
                  data.status === "SHELTERED" || data.status === "CLOSED"
              )
              .map((data) => {
                const {
                  _id,
                ...
                const notify = () =>
                  toast.success("The Sheltering Closed ", {
                    position: "top-right",
                    autoClose: 3000,
